...
var _ = Describe("Gitops functions for managing Snapshots", Ordered, func() {
	...
	Describe("CancelPipelineRuns", func() {
		var (
			...
			testLogger    helpers.IntegrationLogger
		)

		BeforeEach(func() {
			testLogger = helpers.IntegrationLogger{Logger: buflogr.NewWithBuffer(&bytes.Buffer{})}
			...
			inProgressPLR = &tektonv1.PipelineRun{
				ObjectMeta: metav1.ObjectMeta{
					...
					Labels: map[string]string{
						...
						gitops.SnapshotLabel:                    "test-snapshot",
					},
					Annotations: map[string]string{},
		...
		It("removes finalizer from both in-progress and completed PLRs", func() {
			...
			Expect(gitops.CancelPipelineRuns(k8sClient, ctx, testLogger, plrs)).Should(Succeed())
